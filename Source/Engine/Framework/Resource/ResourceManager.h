...
namespace nc
{
	...
	{
		std::string lname = nc::StringUtils::ToLower(name);
	...
	{
		std::string low = nc::StringUtils::ToLower(filename);
		...
		{
			// return resource
			return std::dynamic_pointer_cast<T>(m_resources[low]);
		}

		// resource not in resources map, create resource
		res_t<T> resource = std::make_shared<T>();
		if (!resource->Create(low, args...))
		{
			// resource not created
			WARNING_LOG("Could not create resource: " << low);
			return res_t<T>();
		}
